  ...
  if (resolvedLines.length === 0) {
    return (
      <Container className="py-24 text-center">
        <h1 className="font-serif text-2xl font-semibold text-primary-950">{dict.empty}</h1>
    ...
    <Container className="py-14">
      ...
      <div className="mt-8 grid grid-cols-1 gap-10 lg:grid-cols-3">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 lg:col-span-2">
          ...
          <div>
            <label className="block text-sm font-medium text-primary-950">{dict.phoneOptional}</label>
            <input
              {...register("phone")}
              className="mt-1 w-full rounded-lg border border-primary-200 px-4 py-2.5 focus:border-primary-500 focus:outline-none"
            />
          ...
          <div>
            <label className="block text-sm font-medium text-primary-950">{dict.deliveryAddress}</label>
            <textarea
              {...register("address")}
              rows={3}
              className="mt-1 w-full rounded-lg border border-primary-200 px-4 py-2.5 focus:border-primary-500 focus:outline-none"
          ...
          <div className="rounded-lg border border-primary-200 bg-primary-50/60 p-4 text-sm text-primary-800/80">
            <strong className="text-primary-950">{dict.paymentLabel}</strong> {dict.paymentNote}
          </div>

          {submitError && <p className="text-sm text-red-600">{submitError}</p>}

          <Button type="submit" size="lg" disabled={isSubmitting} className="w-full">
            {isSubmitting ? dict.placingOrder : dict.placeOrder.replace("{total}", formatSom(subtotal, lang))}
          </Button>
        </form>
