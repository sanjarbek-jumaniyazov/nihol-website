...
export default async function AdminOrdersPage() {
  ...
  return (
    <div>
      <h1 className="font-serif text-2xl font-semibold text-primary-950">Orders</h1>
      <p className="mt-1 text-sm text-primary-800/70">
        Marketplace orders and how each was paid. Payment is stubbed until a real gateway
        (Payme/Click) is connected — see src/lib/payment.ts.
      </p>
            ...
            <div key={o.id} className="rounded-2xl border border-primary-100 bg-white p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="text-right">
                  <p className="text-lg font-semibold text-primary-950">{formatSom(o.total_som)}</p>
                  <span
                    className={cn(
                  ...
                  <p className="mt-1 text-xs text-primary-800/50">
                    {o.payment_provider ?? "no provider"}
                ...
                <ul className="mt-4 divide-y divide-primary-100 border-t border-primary-100 pt-3 text-sm">
                  {o.order_items.map((item) => (
                    <li key={item.id} className="flex justify-between py-1.5 text-primary-800/80">
                      ...
                      <span>{formatSom(item.unit_price_som * item.quantity)}</span>
                    </li>
                  ))}
                </ul>
              )}
